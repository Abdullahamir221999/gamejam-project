...
public class Gun : MonoBehaviour
{
    ...
    public void redShoot()
    {
        ...
        bullet.GetComponent<Rigidbody>().velocity = bulletSpawnPoint[0].forward * bulletSpeed *Time.deltaTime;
    }
    public void blueShoot()
    {
        var bullet = Instantiate(bulletPrefab[1], bulletSpawnPoint[1].position, bulletSpawnPoint[1].rotation);
        bullet.GetComponent<Rigidbody>().velocity = bulletSpawnPoint[1].forward * bulletSpeed * Time.deltaTime;
    }
    public void greenShoot()
    {
        var bullet = Instantiate(bulletPrefab[2], bulletSpawnPoint[2].position, bulletSpawnPoint[2].rotation);
        bullet.GetComponent<Rigidbody>().velocity = bulletSpawnPoint[2].forward * bulletSpeed *Time.deltaTime;
    }
